...
export class ManagePluginComponent implements OnInit, OnDestroy {
  ...
  private async getVersionNotes() {
    ...
    if (![this.latestVersion, 'latest'].includes(this.targetVersion)) {
    ...
    const promises = [firstValueFrom(this.$api.get(`/plugins/changelog/${encodeURIComponent(this.pluginName)}`))]
    if (this.versionNotesShow) {
      promises.push(firstValueFrom(this.$api.get(`/plugins/release/${encodeURIComponent(this.pluginName)}`)))
    }

    const results = await Promise.allSettled(promises)

    // Handle changelog result (always the first promise)
    if (results[0].status === 'fulfilled') {
      this.fullChangelog = results[0].value.changelog
    } else {
      console.error('Error loading changelog:', results[0].reason)
      this.fullChangelog = ''
    }

    // Handle version notes result (only if `this.versionNotesShow` is true)
    if (this.versionNotesShow) {
      const versionNotesIndex = 1 // Second promise in the array
      if (results[versionNotesIndex].status === 'fulfilled') {
        if (this.latestVersion.replace(/[^0-9.]/g, '').includes(results[versionNotesIndex].value.name.replace(/[^0-9.]/g, ''))) {
          this.versionNotes = results[versionNotesIndex].value.changelog
        }
      } else {
        console.error('Error loading release notes:', results[versionNotesIndex].reason)
        this.versionNotes = ''
      }
    }

    if (!this.versionNotesShow && this.fullChangelog) {
      this.releaseNotesTab = 2
    }
