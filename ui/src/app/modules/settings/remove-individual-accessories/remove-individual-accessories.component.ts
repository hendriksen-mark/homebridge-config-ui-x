  ...
  async loadCachedAccessories() {
    try {
      ...
      cachedAccessories.forEach((accessory: any) => {
        const bridge = accessory.$cacheFile?.split('.')?.[1] ?? null
        if (bridge) {
          ...
          if (mainPairing) {
            pairingMap.get(mainPairing._id)!.accessories.push(accessory)
          }
        }
      })

      this.pairings = Array.from(pairingMap.values()).filter((pairing: Pairing) => pairing.accessories.length > 0)
      this.selectedBridge = this.pairings[0]?._id ?? ''
